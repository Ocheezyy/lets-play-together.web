...
import { LogIn, Users, GamepadIcon } from "lucide-react"
...
const mockFriends = [
  { id: 1, name: "Alex Johnson", avatar: "/placeholder.svg?height=100&width=100", online: true },
  { id: 2, name: "Sarah Williams", avatar: "/placeholder.svg?height=100&width=100", online: true },
  { id: 3, name: "Mike Brown", avatar: "/placeholder.svg?height=100&width=100", online: false },
  { id: 4, name: "Emma Davis", avatar: "/placeholder.svg?height=100&width=100", online: true },
  { id: 5, name: "James Wilson", avatar: "/placeholder.svg?height=100&width=100", online: false },
  { id: 6, name: "Olivia Taylor", avatar: "/placeholder.svg?height=100&width=100", online: true },
  { id: 7, name: "Daniel Anderson", avatar: "/placeholder.svg?height=100&width=100", online: false },
  { id: 8, name: "Sophia Martinez", avatar: "/placeholder.svg?height=100&width=100", online: true },
]
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white p-4">
        <div className="max-w-md w-full space-y-8 text-center">
          <div className="space-y-4">
            ...
            <p className="text-gray-400">Find out which games you and your friends can play together</p>
          </div>
          <Button size="lg" className="w-full bg-[#1b2838] hover:bg-[#2a475e] text-white" onClick={handleLogin}>
      ...
      <header className="border-b border-gray-800 p-4">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button onClick={clearAuth} variant="secondary">Clear Auth</Button>
            <ThemeToggle />
            <Avatar>
          ...
          <TabsContent value="friends" className="space-y-4">
            <Card className="bg-gray-800 border-gray-700">
              <CardContent className="p-6">
                ...
                <p className="text-gray-400 mb-4">Select the friends you want to play with to see your common games.</p>
                <ScrollArea className="h-[400px] pr-4">
                  <div className="space-y-4">
                    {friends.map((friend) => {
                      const friendSteamId = friend.id;
                      const userName = friend.username || "";
                      const friendAvatar = "";

                      return (
                        <div key={friendSteamId} className="flex items-center space-x-4">
                          <Checkbox
                            id={`friend-${friendSteamId}`}
                            checked={selectedFriends.includes(friendSteamId)}
                            onCheckedChange={() => toggleFriendSelection(friendSteamId)}
                          />
                          <div className="flex items-center space-x-4 flex-1">
                            <Avatar>
                              <AvatarImage src={/* friend.avatar || */ "/placeholder.svg"} alt={userName}/>
                              <AvatarFallback>{userName.charAt(0)}</AvatarFallback>
                            </Avatar>
                            <div className="flex-1">
                              <p className="text-sm font-medium">{userName}</p>
                              {/*<p className={`text-xs ${friend.online ? "text-green-500" : "text-gray-500"}`}>*/}
            ...
            {selectedFriends.length > 0 && (
              <div className="flex justify-end">
                <Button onClick={() => setActiveTab("games")}>View Common Games ({commonGames.length})</Button>
              </div>
            )}
          </TabsContent>
                ...
                {selectedFriends.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {commonGames.map((game) => (
                      <Card key={game.id} className="overflow-hidden bg-gray-700 border-gray-600">
                        ...
                        <CardContent className="p-4">
                          ...
                          <p className="text-xs text-gray-400 mt-1">{game.players.length} friends own this game</p>
                        </CardContent>
                      </Card>
                    ))}
                  </div>
